...
impl LbPairExtension for LbPair {
    ...
    fn compute_fee(&self, amount: u64) -> Result<u64> {
        ...
        let scaled_down_fee = fee.checked_div(denominator).ok_or("overflow").unwrap();

        Ok(scaled_down_fee.try_into()?)
    }
    ...
    fn next_bin_array_index_with_liquidity_internal(
        ...
        if swap_for_y {
            ...
            if offset_bit_map.eq(&U1024::ZERO) {
                return Ok((max_bitmap_id.checked_add(1).ok_or("overflow").unwrap(), false));
            } else {
