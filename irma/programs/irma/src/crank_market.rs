...
pub fn crank_market<'info>( ctx: Context<'_, '_, 'info, 'info, Maint<'info>>, slot: u64 ) -> Result<()> {
    ...
    let consume_events_admin = Signer::try_from(&signer_account_info).unwrap();
    // let event_heap: Account<'info, EventHeap> = Account::try_from(&events_info).unwrap();
    let market: Account<'info, Market> = Account::try_from(&market_info).unwrap();
    ...
    //                     padding: [0u8; 7], // Placeholder for event data
    //                     _padding: [0u64; 17],
    //                 },
    //             }; 30 as usize]; 20], // just the first 600 events
    //             reserved: [0u8; 64],
    //         };
    //         heap.nodes[0][0].event.event_type = 1; // Set the first event type to 1
    //         return heap;
    //     }
    ...
    // msg!("consume_given_events completed, event heap updated: {:?}", event_heap_buf);
...
fn prep_accounts<'info>(
    ...
    let market_buffer = [0u8; 8 + std::mem::size_of::<Market>()];
    let market_buf_ref: &'info mut [u8] = Box::leak(Box::new(market_buffer)); // &mut market_buffer;
